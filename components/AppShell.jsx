...
export default function AppShell({ initialPanel = "home", initialTeam = null, initialYear = null, initialPlayer = null }) {
  ...
  const handleRosterUpload = (csvText) => {
  ...
  const handleClearGameLogs = () => updateData({ gameLogs: [] });
  ...
  const handleAddManualResult = (result) => {
    setAppData((prev) => {
      const cur = getSeasonData(prev, currentSeason);
  ...
  const handleRemoveManualResult = (id) => {
    setAppData((prev) => {
      const cur = getSeasonData(prev, currentSeason);
      const existing = cur.manualResults || [];
  ...
  const handleApplyRenames = (renames) => {
    setAppData((prev) => {
      const cur = getSeasonData(prev, currentSeason);
  ...
  const handleSignPlayer = (playerId, targetTeam) => {
  ...
  const handleChangePosition = (playerId, newPosId) => {
    setAppData((prev) => storeChangePosition(prev, playerId, newPosId, currentSeason));
  };

  const handleChangeJersey = (playerId, newNum) => {
  ...
  useEffect(() => {
    ...
    const target = panel === "standings" ? "/" : `/${panel}/`;
    if (pathname !== target && pathname !== target.replace(/\/$/, "")) {
      router.replace(target, { scroll: false });
    }
  }, [panel, pathname, router]);
  ...
  const renderPanel = () => {
    ...
    switch (panel) {
      ...
      case "teams":
        return <TeamsPage {...sharedProps} selectedTeam={selectedTeam} />;
      case "schedules":
        return <SchedulesPage {...sharedProps} schedule={schedule} gameLogs={gameLogs} onUploadGameLog={handleGameLogUpload} onClearGameLogs={handleClearGameLogs} />;
      case "leaders":
...
function NavBar({ panel, setPanel, duplicateCount, isAdmin }) {
  ...
  return (
    <div ref={containerRef} style={{ display: "flex", alignItems: "stretch", overflowX: "auto", background: "#0c0c0c" }}>
      {groups.map((g) => {
        ...
        return (
          <div key={g.id} style={{ position: "relative" }}>
            <button
              ...
              onMouseEnter={() => { if (openId) setOpenId(g.items ? g.id : null); }}
              style={{
                ...itemBase,
